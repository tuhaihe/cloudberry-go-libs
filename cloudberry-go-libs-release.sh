...
# ======================================================================
#
# cloudberry-go-libs-release.sh — Cloudberry Go-libs (Incubating) release utility
#
...
#
# Supported Features:
#   - Derives the release version directly from the Git tag
...
#
# Usage:
#   ./cloudberry-go-libs-release.sh --stage --tag 2.0.0-incubating-rc1 --gpg-user your@apache.org
#
# Options:
...
#   -r, --repo <path>         Optional path to local cloudberry-go-libs Git repository
...
#
# Requirements:
#   - Must be run from the root of a valid cloudberry-go-libs Git clone,
#     or the path must be explicitly provided using --repo
#   - Git user.name and user.email must be configured
#   - Repository remote must be: git@github.com:apache/cloudberry-go-libs.git
#   - Required tools: sha512sum, tar (gtar on macOS), gpg, xmllint
#   - On macOS: brew install coreutils gnu-tar gnupg
#
# Examples:
#   ./cloudberry-go-libs-release.sh -s -t 2.0.0-incubating-rc1 --gpg-user your@apache.org
#   ./cloudberry-go-libs-release.sh -s -t 2.0.0-incubating-rc1 --skip-signing
#   ./cloudberry-go-libs-release.sh --stage --tag 2.0.0-incubating-rc2 --force-tag-reuse --gpg-user your@apache.org
#   ./cloudberry-go-libs-release.sh --stage --tag 2.0.0-incubating-rc1 -r ~/cloudberry-go-libs --skip-remote-check --gpg-user your@apache.org
#
# Notes:
#   - When reusing a tag, the `--force-tag-reuse` flag must be provided.
#   - The release version is always derived from the Git tag; no VERSION file is needed.
# ======================================================================
...
show_help() {
  echo "Cloudberry Go-libs (Incubating) Release Tool"
  echo
  echo "Usage:"
  ...
  echo
  echo "  -r, --repo <path>"
  echo "      Optional path to a local cloudberry-go-libs Git repository clone"
  echo
  echo "  -S, --skip-remote-check"
  echo "      Skip remote.origin.url check (use for forks or mirrors)"
  echo "      Required for official releases:"
  echo "        git@github.com:apache/cloudberry-go-libs.git"
  echo
  echo "  -g, --gpg-user <key>"
...
if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  echo "Run from a cloudberry-go-libs clone or pass --repo <path>."
  exit 1
fi
...
# Ensure we are in a valid cloudberry-go-libs source directory
if [[ ! -f go.mod ]]; then
  echo "ERROR: '$REPO_ROOT' does not look like a valid cloudberry-go-libs source root."
...
if [[ "$SKIP_REMOTE_CHECK" != true ]]; then
  ...
  if [[ "$REMOTE_URL" != "git@github.com:apache/cloudberry-go-libs.git" ]]; then
    echo "ERROR: remote.origin.url must be 'git@github.com:apache/cloudberry-go-libs.git' for official releases."
...
section "Staging release: $TAG"
  ...
  if [[ "$FORCE_TAG_REUSE" == false ]]; then
    ...
    git tag -a "$TAG" -m "Cloudberry Go-libs (Incubating) ${TAG} Release Candidate"
  else
  ...
  TAR_NAME="apache-cloudberry-go-libs-${VERSION_FILE}-incubating-src.tar.gz"
  TMP_DIR=$(mktemp -d)
  ...
  git archive --format=tar --prefix="apache-cloudberry-go-libs-${VERSION_FILE}-incubating/" "$TAG" | tar -x -C "$TMP_DIR"
  ...
  if [ -s .gitmodules ]; then
    git submodule foreach --recursive --quiet "
      ...
      destpath=\"$TMP_DIR/apache-cloudberry-go-libs-${VERSION_FILE}-incubating/\$sm_path\"
      mkdir -p \"\$destpath\"
      git -C \"\$fullpath\" archive --format=tar --prefix=\"\$sm_path/\" HEAD | tar -x -C \"$TMP_DIR/apache-cloudberry-go-libs-${VERSION_FILE}-incubating\"
    "
  fi
  ...
  if [[ "$DETECTED_PLATFORM" == "macOS" ]]; then
    ...
    if command -v xattr >/dev/null 2>&1; then
      find "$TMP_DIR/apache-cloudberry-go-libs-${VERSION_FILE}-incubating" -type f -exec xattr -c {} \; 2>/dev/null || true
      echo "[OK] Extended attributes cleaned using xattr"
    fi
    
    # Remove any ._* files that might have been created
    find "$TMP_DIR/apache-cloudberry-go-libs-${VERSION_FILE}-incubating" -name '._*' -delete 2>/dev/null || true
    find "$TMP_DIR/apache-cloudberry-go-libs-${VERSION_FILE}-incubating" -name '.DS_Store' -delete 2>/dev/null || true
    find "$TMP_DIR/apache-cloudberry-go-libs-${VERSION_FILE}-incubating" -name '__MACOSX' -type d -exec rm -rf {} \; 2>/dev/null || true
  ...
  if [[ "$DETECTED_PLATFORM" == "macOS" ]]; then
    ...
    $DETECTED_TAR_TOOL --exclude='._*' --exclude='.DS_Store' --exclude='__MACOSX' -czf "$TAR_NAME" -C "$TMP_DIR" "apache-cloudberry-go-libs-${VERSION_FILE}-incubating"
    echo "INFO: macOS detected - applied extended attribute cleanup and GNU tar"
  else
    # On other platforms, use standard tar
    $DETECTED_TAR_TOOL -czf "$TAR_NAME" -C "$TMP_DIR" "apache-cloudberry-go-libs-${VERSION_FILE}-incubating"
  fi
